...
SIMION_INSTALL_DIR = pathlib.Path(r"C:\Users\Administrador\Downloads\SIMION")
...
OPTIMIZE = {
    ...
    9:  (0.0, 1000.0),   # Quadrupole bender
    10: (0.0, 1000.0),   # Quadrupole bender
    11: (0.0, 1000.0),   # Quadrupole bender
    12: (-1000.0, 0.0),   # Quadrupole bender
...
# WHAT IS THIS AND HOW TO USE IT?  See the Optuna docs for `enqueue_trial()`. 
# You can use this to start the optimization from a known-good set of voltages, or leave it as None to start from scratch. Example:
STARTING_POINT = {"V3": -121.75456483424807, "V6": -74.38995609381641, "V9": 998.4993982155488, "V10": 269.46637797947216, "V11": 287.37549712784056, "V12": -131.60004105893447, "V15": 83.10820852745206, "V18": -471.7782392345852}
# STARTING_POINT = None
...
#       f'... "{IOB_FILE}"'
FLY_COMMAND = f'simion --nogui fly --recording-output=out.txt --programs=0 --retain-trajectories=0 --restore-potential=0 "{IOB_FILE}"'  # TODO (Task 1a)
...
def apply_voltages(chosen: dict) -> None:
    ...
    """
    all_volts = {**FIXED, **chosen} # TODO (Task 1b): replace this with your code. -- raise NotImplementedError("Task 1b: build and run the fastadj command")
    settings = ",".join(f"{n}={v}" for n, v in sorted(all_volts.items()))
    command = f'simion --nogui fastadj "{PA0_FILE}" {settings}'
    run_simion(command)
...
def objective(trial: Trial) -> float:
    ...
    chosen = {} #this dictionary will hold the chosen voltages for each electrode
    for number, (low, high) in OPTIMIZE.items():
        chosen[number] = trial.suggest_float(f"V{number}", low, high) # this is the correct way to suggest a float value for each electrode in the optimization range
    try:
        apply_voltages(chosen)
        simion_output = run_simion(FLY_COMMAND)
    ...
    if positions.shape[0] == 0:
        return BAD_SCORE
    if OBJECTIVE == "hits":
        return count_hits(positions)
    else:
        return beam_spread(positions) 

def main() -> None:
    print("Beamline voltage optimizer (SIMION + Optuna) - STUDENT VERSION")
    print(f"Goal: {'MAXIMIZE ions on detector' if OBJECTIVE == 'hits' else 'MINIMIZE beam spread'}") #This line changes if the OBJECTIVE variable does.
    ...
    # raise NotImplementedError("Task 2b: create the study and call study.optimize()")
    # ==========================================================================
    study = optuna.create_study(
        direction=DIRECTION,
        storage=f"sqlite:///{RESULTS_DB}",
